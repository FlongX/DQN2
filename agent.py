...
class Agent(object):
    def __init__(self, args, save_model_path, device, test_log_path=None):
        ...
        self.policy_net = DQN(10, self.n_actions)
        self.target_net = DQN(10, self.n_actions)
        ...
        self.policy_net = self.policy_net.cuda(device=device)
        self.target_net = self.target_net.cuda(device=device)

        params = list(self.policy_net.parameters())
        self.optimizer = optim.Adam(params, lr=args.lr)

        self.memory = ReplayMemory(args.memory)
        self.episode_durations = []  # 回合持续时间

        self.actions_history = []
        self.num_episodes = args.num_episodes
        self.actions_history += [[100] * 11] * 20
    ...
    @staticmethod
    def compute_reward(action, actual_box, previous_box, gt):
        coor = np.nonzero(gt)
        hmin = min(coor[0])
        hmax = max(coor[0])
        wmin = min(coor[1])
        ...
        b_gt_h = hmin + (hmax - hmin) / 2
        b_gt_w = wmin + (wmax - wmin) / 2
        b_actual_box_h = actual_box[2] + (actual_box[3] - actual_box[2]) / 2
        b_actual_box_w = actual_box[0] + (actual_box[1] - actual_box[0]) / 2
        b_previous_box_h = previous_box[2] + (previous_box[3] - previous_box[2]) / 2
        b_previous_box_w = previous_box[0] + (previous_box[1] - previous_box[0]) / 2

        if 6 <= action <= 9:
            r = np.sign(math.sqrt((b_previous_box_h - b_gt_h) ** 2 + (b_previous_box_w - b_gt_w) ** 2) - math.sqrt(
                (b_actual_box_h - b_gt_h) ** 2 + (b_actual_box_w - b_gt_w) ** 2)
                        )
            # if r == 0:
            #     print(action, b_gt_w, b_gt_h, r, b_previous_box_h, b_previous_box_w , b_actual_box_h , b_actual_box_w, '\n')
        else:
            if 0 <= action <= 5:
                if IOU(previous_box, gt) == 0 and IOU(actual_box, gt) == 0 and (previous_box[3] - previous_box[2]) > (
                        (hmax - hmin) * 0.9) and (previous_box[1] - previous_box[0]) > ((wmax - wmin) * 0.9):

                    r = np.sign(math.sqrt((b_previous_box_h - b_gt_h) ** 2 + (b_previous_box_w - b_gt_w) ** 2) -
                                math.sqrt((b_actual_box_h - b_gt_h) ** 2 + (b_actual_box_w - b_gt_w) ** 2)
                                )
                else:
                    r = np.sign(IOU(actual_box, gt) - IOU(previous_box, gt))
        return r
    ...
    def get_best_next_action(self, actions, gt):  # 探索阶段不适用Q_net选择动作，从积极动作集中选一个最好的
        ...
        if ((b_actual_box_h - b_gt_h) ** 2 + (b_actual_box_w - b_gt_w) ** 2) > ((wmax-wmin) ** 2 + (hmax-hmin)**2) ** 0.5 / 8:
            return random.randint(6, 10)
        else:
            if (actual_box[3] - actual_box[2]) > ((hmax - hmin) * 0.9) or (actual_box[1] - actual_box[0]) > ((wmax - wmin) * 0.9):
                return random.randint(0, 6)
            else:
                return random.randint(10, 10)

    def select_action(self, state, actions, gt):
        if self.eps_priori<=0:
            self.eps_priori=0
        eps = self.eps
        eps_priori = self.eps_priori

        if random.random() < eps_priori:
    ...
    def optimize_model(self, episodes, n_batch):
        ...
        state_batch = state_batch.cuda(device=device)
        action_batch = torch.LongTensor(batch.action).view(-1, 1).type(LongTensor).cuda(device=device)
        reward_batch = torch.FloatTensor(batch.reward).view(-1, 1).type(Tensor).cuda(device=device)
        next_states_values = torch.zeros(self.batch_size, 1).type(Tensor).cuda(device=device)

        # 获取策略网络中和action_batch对应的q值
        state_action_values = self.policy_net(state_batch).gather(1, action_batch)  # V(s(t))

        non_final_next_states = non_final_next_states.cuda(device=device)
        d = self.target_net(non_final_next_states)  # V(s(t+1))
        next_states_values[non_final_mask] = d.max(1)[0].view(-1, 1)

        # Q(s(t), at) = gamma*max_a(Q'(s(t+1), a)) + r
        expected_state_action_values = (next_states_values * self.gamma) + reward_batch
        loss = sum(abs(state_action_values - expected_state_action_values))/len(state_action_values)

        # loss = loss + l2_regularization(self.policy_net, 0.0004)
    ...
    def compose_state(self, g_image, l_image, step, dtype=FloatTensor):
        """
             状态组合 = 图片特征向量 + 历史动作集
        """
        g_image_feature, l_image_feature = self.get_features(g_image, l_image, step)  # 获取从图片中提取得到的特征
        g_image_feature = g_image_feature.view(1, -1)  # 修改维度为1行
        l_image_feature = l_image_feature.view(1, -1)  # 修改维度为1行

        history_flatten = self.actions_history.view(1, -1).type(dtype)
        history_flatten = history_flatten.cuda(device=device)

        state = torch.cat((g_image_feature, l_image_feature, history_flatten), 1)  # 按列拼接(左右拼接)
        # print(g_image_feature.shape, l_image_feature.shape, history_flatten.shape, box_feature.shape, state.shape)
        return state

    def get_features(self, g_image, l_image, step, dtype=FloatTensor):
        global transform
        if step == 0:
            g_image = g_image.view(1, *g_image.shape)
            # 使用*将image.shape中的数据直接读出(image.shape是使用张量存储的数据); 使用view重构,(1,3,224,224)
            g_image = g_image.type(dtype).cuda(device=device)
            self.g_feature = self.feature_extractor(g_image)

        l_image = l_image.view(1, *l_image.shape)
        # 使用*将image.shape中的数据直接读出(image.shape是使用张量存储的数据); 使用view重构,(1,3,224,224)
        l_image = l_image.type(dtype).cuda(device=device)

        l_feature = self.feature_extractor(l_image)

        return self.g_feature.data, l_feature.data

    def update_history(self, action):
        """
            添加新的动作到历史动作集(共保存11个动作组)
            参数 :
                - 动作索引
        """
        ...
        if size_history_vector < 10:
            self.actions_history[size_history_vector][action] = 1  # 不足10个动作 -> 将动作添加到最后一个动作中(最后一个动作集中动作action设为1)

        return self.actions_history

    def local_box(self, g_image, l_image):

        self.policy_net.load_state_dict(torch.load(self.save_path, map_location=torch.device(device=0)))
        # Q-Network转换到评估模式
        self.policy_net.eval()

        done = False
        all_actions = []
        self.actions_history = torch.zeros((10, 11))

        original_image = g_image.clone()

        # 执行10步 或 选择动作0
        steps = 0
        state = self.compose_state(g_image, l_image, steps)
        while not done:
            steps += 1
            ...
            if action == 10:
                ...
                new_image = original_image[:, int(new_equivalent_coord[2]):int(new_equivalent_coord[3]),
                            int(new_equivalent_coord[0]):int(new_equivalent_coord[1])]  # 原图（c, h, w）
                try:
                    new_image = transform(new_image)
                except ValueError:
                    break

                next_state = self.compose_state(original_image, new_image, steps)
        ...
        box = [fin_xmin, fin_xmax, fin_ymin, fin_ymax]

        return box


    def predict_image(self, g_image, l_image, media_path, gt, plot=True):
        """
            预测图像的标注框
        """
    ...
    def save_network(self, epoch):
        """
            保存Q-Network
        """
        policy_path = os.path.join(self.save_path, f'policy{epoch}.pth')
        torch.save(self.policy_net.state_dict(), policy_path)

        self.logger.info(f'Saved policy and feature at epoch {epoch}')
    ...
    def train(self):
        ...
        db_train = self.args.dataset(base_dir=self.args.root_path, split="train",outsize=self.args.img_size,
                           transform=transforms.Compose(
                               [RandomGenerator(split='train', output_size=[self.args.img_size, self.args.img_size])]))
        ...
        for i_episode in iterator:
            ...
            for sampled_batch in trainloader:
                ...
                #original_l_image = image_batch[:, 65:159, 65:159]
                original_l_image = image_batch[:, 0:224, 0:224]
                original_l_image = transform(original_l_image)
                ...
                self.actions_history = torch.zeros((10, 11))
                 # 状态 = 图片特征向量 + 动作历史
                #original_coordinates = [65, 159, 65, 159]  # 初始的标注框，按照原论文，初始框在中心，大小原图42%
                original_coordinates = [0, 224, 0, 224]
                done = False  # 一回合结束标志
                t = 0
                state = self.compose_state(image_batch, original_l_image, t)
                actual_equivalent_coord = original_coordinates  # 实际标注框初始化

                while not done:  # 结束一张图片的标志：执行15次动作 或 action=10
                    t += 1
                    action = self.select_action(state, all_actions, gt)  # 选择一个动作
                    ...
                    if action == 10:  # 结束标志
                        ...
                        next_state = self.compose_state(original_image, new_image,t)  # 状态组合 = 图片特征向量 + 动作历史
                        reward = self.compute_reward(action, new_equivalent_coord, actual_equivalent_coord,
                ...
                if n_batch % (int(len_set / 10)) == 0:
                    float_reward = rew.item() if isinstance(rew, torch.Tensor) else float(rew)
    ...
    def test(self):
        ...
        self.policy_net.load_state_dict(
            torch.load(self.save_path + '/policy24.pth', map_location=torch.device(device=device)))
        ...
        for test_case in test_case_dir:
            if 66 < int(test_case):
                ...
                for sampled_batch in testloader:
                    ...
                    original_l_image = image_batch[:, 0:224, 0:224]
                    #original_l_image = image_batch[:, 0:224, 0:224]
                    l_image = transform(original_l_image)

                    bbox, t = self.predict_image(image_batch, l_image, os.path.join(media_path, test_case), label_batch)  # 预测图像的标注框
