from config import create_parser as agent_create_parser
...
device = 0
save_model_path = r'DQN2/model/DRL_NIH224/DRL_epo25_bs128_lr0.0005_s1234/policy24.pth'
parer1 = agent_create_parser()
args1 = parer1.parse_args()
agent = Agent(args1, save_model_path, device, './')
open('box_data.csv', 'w').close()
#
# base_dir = r'./Data/Pancreas/'
base_dir = r'./Data/NIH/'
save_dir = r'../data/local_data'
...
for case in case_dir:
    ...
    save_image_path = os.path.join(save_dir, 'image', case)
    save_label_path = os.path.join(save_dir, 'label', case)
    ...
    for num in os.listdir(case_image_path):
        ...
        box = agent.local_box(image, image)
        box_list.append(box)
        num_list.append(num)
